"""manual membership payment flow
...
def upgrade() -> None:
    op.add_column("users", sa.Column("paid_membership_expires_at", sa.DateTime(timezone=True), nullable=True))
    op.add_column("users", sa.Column("paid_membership_plan_code", sa.String(length=32), nullable=True))
...
def downgrade() -> None:
    ...
    op.drop_column("users", "paid_membership_plan_code")
    op.drop_column("users", "paid_membership_expires_at")
